...
def main():
    while True:
        ...
        gender = {'him', 'her'}
        if command and 'message' in command and any(g in command for g in gender):
            # Extracting person and message from the command
            parts = command.split('message')[1].strip().split(' and tell ' or 'and ask' or 'tell')
            print(parts)
            
            if len(parts) >= 2:  # Check if 'parts' has at least 2 elements
                for g in gender:
                    if g in parts[1]:
