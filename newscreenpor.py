...
def capture_website_text(url):
    ...
    # Check if the word "login" is present in any of the stored text
    login_present = any("coding for data" in entry["text"].lower() for entry in text_positions)

    # Print "Yes" and the position if the word "login" is present
    if login_present:
        login_positions = [entry["position"] for entry in text_positions if "login" in entry["text"].lower()]
        print("Yes, 'login' is present at the following positions:")
        for index, position in enumerate(login_positions, start=1):
            print(f"Position {index}: {position}")
    else:
        print("No, 'login' is not present.")
